...
ratio = OddToEvenHarmonicEnergyRatio()
entropy = Entropy()
hpcp = HPCP()
pitch = PitchYinFFT()
...
f.write('@ATTRIBUTE f0-mean REAL\n')
f.write('@ATTRIBUTE f0-variance REAL\n')
f.write('@ATTRIBUTE entropy-mean REAL\n')
f.write('@ATTRIBUTE entropy-variance REAL\n')
f.write('@ATTRIBUTE harmonicratio-mean REAL\n')
f.write('@ATTRIBUTE harmonicratio-variance REAL\n')
...
for audio_file in dirList:
	...
	for frame in FrameGenerator(audio, frameSize = 1024, hopSize = 512, startFromZero=True):
		...
		f0.append(pitch(spectrum(w(frame)))[0])
		ent.append(entropy(spectrum(w(frame))))
		rat.append(ratio(freq[0][0], freq[0][1]))
	...
	f.write(',')
	f.write('%r' %np.mean(f0))
	f.write(',')
	f.write('%r' %np.var(f0))
	f.write(',')
	f.write('%r' %np.mean(ent))
	f.write(',')
	f.write('%r' %np.var(ent))
	f.write(',')
	f.write('%r' %np.mean(rat))
	f.write(',')
	f.write('%r' %np.var(rat))
	f.write(',')
